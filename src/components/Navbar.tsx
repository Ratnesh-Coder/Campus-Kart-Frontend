          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex-shrink-0">
              <Link to="/" className="flex items-center space-x-2 group">
                <img src={CampusKart} alt="Campus Kart Logo" className="h-10 w-auto transition-transform duration-200 ease-in-out hover:scale-110"/>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white pb-4 px-4 space-y-2">
             <div className="relative w-full mb-4">
              ...
              {user ? (
                <>
                  <Link to="/profile" onClick={() => setMobileMenuOpen(false)} className="block py-2 font-semibold text-neutral-700 hover:bg-neutral-100">
                  <UserIcon /><span className="ml-3">My Profile</span>
                  </Link>
                  <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="flex items-center w-full text-left py-2 px-2 rounded-md font-semibold text-neutral-700 hover:bg-neutral-100">
                  <LogoutIcon /> <span className="ml-3">Logout</span>
                  </button>
                </>
              ) : (
                 <button onClick={() => { setLoginPop(true); setMobileMenuOpen(false); }} className="flex items-center w-full text-left py-2 px-2 rounded-md font-semibold text-neutral-700 hover:bg-neutral-100">Login / Sign Up
                 </button>
              )}
          </div>
        )}
